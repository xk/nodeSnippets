...
stdin.addListener('data', function (chunk, newLinePos) {  //los datos van llegando por aquí
...
var ctr= 0;
var buffer= "";
function procesarBuffer () {                              //Esto se ejecuta mientras estamos esperando a que lleguen más datos
  while ((newLinePos= buffer.indexOf("\n")) >= 0) {       //Vamos cortando por los saltos de línea: esto es una especie de readLine()
    ctr+= ER.test(buffer.substr(0, newLinePos));          //al sumar true/false a un 'number' se convierte en 1/0
